...
test_that("compute_time_trend counts rows when measure_col is NULL", {
  result <- govhr::compute_time_trend(test_data, group = "ref_date")
...
test_that("compute_time_trend sums measure_col when supplied", {
  result <- govhr::compute_time_trend(test_data, group = "group", measure_col = "salary")
...
test_that("rescale_baseline sets first period to 100 when ungrouped", {
  summary <- govhr::compute_time_trend(test_data, group = "ref_date")
  result <- govhr::rescale_baseline(summary, group = "ref_date")
  expect_equal(result$value[result$ref_date == min(result$ref_date)], 100)
})

test_that("rescale_baseline rescales independently per group", {
  summary <- govhr::compute_time_trend(test_data, group = "group", measure_col = "salary")
  result <- govhr::rescale_baseline(summary, group = "group")
  first_vals <- result |>
...
test_that("compute_cross_section_summary uses only the latest date", {
  result <- govhr::compute_cross_section(test_data, group = "group")
...
test_that("compute_cross_section_summary sums measure_col at latest date", {
  result <- govhr::compute_cross_section(test_data, group = "group", measure_col = "salary")
...
test_that("compute_growth_summary returns growth_rate column with correct sign", {
  result <- govhr::compute_growth(test_data, measure_col = "salary", group = "group")
...
test_that("compute_growth_summary computes correct pct change with measure_col", {
  result <- govhr::compute_growth(test_data, group = "group", measure_col = "salary")
...
test_that("plot_bar_total returns a ggplot with a GeomCol layer", {
  cs <- govhr::compute_cross_section(test_data, group = "group")
...
test_that("plot_bar_growth returns a ggplot with a GeomVline layer at 0", {
  growth <- govhr::compute_growth(test_data, group = "group", measure_col = "salary")
