...
test_that("compute_trend_summary counts rows when measure_col is NULL", {
  result <- compute_trend_summary(test_data, group_col = "ref_date")
...
test_that("compute_trend_summary sums measure_col when supplied", {
  result <- compute_trend_summary(test_data, group_col = "group", measure_col = "salary")
...
test_that("apply_baseline_index sets first period to 100 when ungrouped", {
  summary <- compute_trend_summary(test_data, group_col = "ref_date")
  result <- apply_baseline_index(summary, group_col = "ref_date")
...
test_that("apply_baseline_index rescales independently per group", {
  summary <- compute_trend_summary(test_data, group_col = "group", measure_col = "salary")
  result <- apply_baseline_index(summary, group_col = "group")
  first_vals <- result |>
...
test_that("compute_cross_section_summary uses only the latest date", {
  result <- compute_cross_section_summary(test_data, group_col = "group")
...
test_that("compute_cross_section_summary sums measure_col at latest date", {
  result <- compute_cross_section_summary(test_data, group_col = "group", measure_col = "salary")
...
test_that("compute_growth_summary returns growth_rate column with correct sign", {
  result <- compute_growth_summary(test_data, measure_col = "salary", group_col = "group")
...
test_that("compute_growth_summary computes correct pct change with measure_col", {
  result <- compute_growth_summary(test_data, group_col = "group", measure_col = "salary")
...
test_that("plot_trend returns a ggplot object", {
  summary <- compute_trend_summary(test_data, group_col = "ref_date")
  expect_s3_class(plot_trend(summary, group_col = "ref_date"), "ggplot")
})

test_that("plot_trend toggle_growth adds a hline layer", {
  summary <- compute_trend_summary(test_data, group_col = "ref_date")
  p <- plot_trend(summary, group_col = "ref_date", toggle_growth = TRUE)
...
test_that("plot_bar_total returns a ggplot with a GeomCol layer", {
  cs <- compute_cross_section_summary(test_data, group_col = "group")
  p <- plot_bar_total(cs, group_col = "group", x_label = "Headcount")
  layer_geoms <- sapply(p$layers, \(l) class(l$geom)[1])
...
test_that("plot_bar_growth returns a ggplot with a GeomVline layer at 0", {
  growth <- compute_growth_summary(test_data, group_col = "group", measure_col = "salary")
  p <- plot_bar_growth(growth, group_col = "group")
  expect_s3_class(p, "ggplot")
  layer_geoms <- sapply(p$layers, \(l) class(l$geom)[1])
